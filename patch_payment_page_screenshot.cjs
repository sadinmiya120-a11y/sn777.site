...
for (const file of files) {
  ...
  let pEnd = code.indexOf(key);
  if (pEnd !== -1) {
    let pEndFull = pEnd + key.length;
    ...
    if (pStart !== -1) {
      ...
      code = code.substring(0, pStart) + exactScreenshotPaymentScreen + code.substring(pEndFull);
    }
  }
  ...
  try {
    ...
    console.log("Successfully patched payment page in:", file);
  } catch (e) {
