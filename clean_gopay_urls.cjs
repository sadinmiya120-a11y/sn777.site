...
jsFiles.forEach(filePath => {
  ...
  // Match 2: Le=`...`
  code = code.replace(
    /Le=`\$\{.*?\}(https:\/\/[^\/]*)?\/gopay_pay\.php\?uid=\$\{gt\.currentUser\.uid\}&amount=\$\{E\}&method=\$\{Be\}&order_no=\$\{Te\}`/g,
    `Le=\`${BACKEND_URL}/gopay_pay.php?uid=\${gt.currentUser.uid}&amount=\${E}&method=\${Be}&order_no=\${Te}\``
  );

  // Match 3: window.location.href=`...`
  code = code.replace(
    /window\.location\.href=`\$\{.*?\}(https:\/\/[^\/]*)?\/gopay_pay\.php\?uid=\$\{gt\.currentUser\.uid\}&amount=\$\{E\}&method=\$\{Be\}&order_no=\$\{Te\}`/g,
